 ...
 */
 ...
 func processSource(logger *log.Logger, nsCfg *config.NamespaceConfig, t tail.Follower, parser parser.Parser, metrics *metrics.Collection, hasCounterOnlyLabels bool) error {
	 ...
	 for line := range t.Lines() {
		 ...
		 if method, ok := fields["request_method"]; ok {
			 metrics.RequestMethodTotal.WithLabelValues(method).Inc()
		 }
		 if status, ok := fields["status"]; ok {
			 metrics.ResponseStatusCodeTotal.WithLabelValues(status).Inc()
		 }
		 if remoteAddr, ok := fields["remote_addr"]; ok {
			 metrics.RemoteAddressTotal.WithLabelValues(remoteAddr).Inc()
		 }
		 metrics.TotalRequestNumber.Inc()
	 }
	 return nil
 }
